...
app = Flask(__name__)
CORS(app, resources={r"/predict": {"origins": "*"}})  # Be cautious when using "*" in production
...
def predict():
    try:
        ...
        if predicted_class is not None:
            ...
            return jsonify(class_label=predicted_class, accuracy= float(accuracy)  , image_path= image_url)
        else:
